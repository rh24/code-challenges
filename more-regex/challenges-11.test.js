...
const findTagNames = elements => [].concat.apply([], elements.map(tag => tag.match(/\<\/\w+\>/gi).map(el => el.replace(/[\<\>]/g, ''))));
...
const validatePhoneNumber = (phoneNumber) => /^(\(\d{3}\)\s?)(\d{3})\s?|\-?(\d{4})|((\d{3}\-))(\d{3}\-)(\d{4})|(\d{3})\s?(\d{3})\s?|\-?(\d{4})$/.test(phoneNumber);

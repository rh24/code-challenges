...
// CLASS EXERCISE: write our own map
...
console.log(myMap([2, 4, 6, 8], callback));

const forLoopTwoToThe = (input) => {
  const result = [];

  for (let i = 0; i < input.length; i++) {
    result.push(Math.pow(2, input[i]));
  }
...
const forEachTwoToThe = (input) => {
  ...
  input.forEach(n => result.push(Math.pow(2, n)));
...
const mapTwoToThe = (input) => {
  return input.map(n => Math.pow(2, n));
}
...
const charCode = (input) => {
  return input.map((char, idx) => char.charCodeAt(0));
};

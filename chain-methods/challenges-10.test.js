...
const count = (target, input) => input.reduce((acc, curr) => acc + curr.filter(n => n === target).length, 0);
...
const divisibleByFiveTwoToThePower = (input) => input.map(n => n.filter(n => n % 5 === 0 && typeof n === 'number').map(n => 2 ** n));
